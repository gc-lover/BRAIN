...
- Сервисы MVP: `auth-service`, `player-service`, `gameplay-service`, `social-service`, `economy-service`, `world-service`, `api-gateway`.

## 2. Модули и ответственность

| Сервис | Порт | Хранилище | Ответственность |
|--------|------|-----------|-----------------|
| auth-service | 8080 | PostgreSQL (`auth` schema) | Регистрация, OAuth2, refresh токены |
| player-service | 8081 | PostgreSQL (`mvp_core`) | Аккаунты, персонажи, инвентарь |
...
| api-gateway | 8088 | N/A | BFF, маршрутизация, rate limiting |
