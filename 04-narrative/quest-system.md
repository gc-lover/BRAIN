# Система квестов NECPGAME

**Статус:** draft  
**Версия:** 1.0.0  
**Дата создания:** 2025-11-03  
**Последнее обновление:** 2025-11-03  
**Приоритет:** высокий

**api-readiness:** needs-work  
**api-readiness-check-date:** 2025-11-03 21:15  
**api-readiness-notes:** Документ в процессе брейншторма, требуется детализация механик квестов

---

## Краткое описание

Система квестов для NECPGAME, включающая основные, побочные и ежедневные квесты с интеграцией системы выбора и последствий.

---

## Вопросы для проработки

### Q1: Типы квестов

**Вопрос:** Какие типы квестов должны быть в игре?

**Варианты:**

**A. Классическая система MMORPG**
- Основные квесты (main quests) - главный сюжет
- Побочные квесты (side quests) - дополнительные истории
- Ежедневные квесты (daily quests) - повторяемые задачи
- Еженедельные квесты (weekly quests) - долгосрочные цели

**B. Расширенная система**
- Основные квесты (main quests)
- Побочные квесты (side quests)
- Ежедневные квесты (daily quests)
- Еженедельные квесты (weekly quests)
- Квесты фракций (faction quests) - задания от фракций
- Клановые квесты (guild quests) - задания для кланов
- Динамические квесты (dynamic quests) - события мира

**C. Гибридная система**
- Основные квесты (main quests)
- Побочные квесты (side quests)
- Ежедневные квесты (daily quests)
- Еженедельные квесты (weekly quests)
- Квесты фракций (faction quests)
- Клановые квесты (guild quests)
- Динамические квесты (dynamic quests)
- Эвентовые квесты (event quests) - связанные с мировыми событиями
- Квесты социальных механик (social quests) - отношения, романсы, семья

**D. Собственный вариант**
- [описание своей идеи]

---

### Q2: Структура основных квестов

**Вопрос:** Как должны быть структурированы основные квесты (main quests)?

**Варианты:**

**A. Линейная прогрессия**
- Один главный сюжет, который развивается линейно
- Игрок следует по сюжетной линии без выбора
- Простота реализации, но меньше реиграбельности

**B. Ветвящийся сюжет**
- Главный сюжет с множеством веток
- Выборы игрока влияют на развитие сюжета
- Разные концовки в зависимости от выборов
- Высокая реиграбельность

**C. Мультисюжетная система**
- Несколько независимых основных сюжетных линий
- Игрок может выбирать какую линию проходить
- Линии могут пересекаться и влиять друг на друга
- Максимальная свобода выбора

**D. Гибридная система**
- Несколько основных сюжетных линий
- Каждая линия имеет ветвление
- Линии пересекаются и влияют друг на друга
- Разные концовки в зависимости от выборов
- Связь с системой лиг (каждая лига = новая история)

**E. Собственный вариант**
- [описание своей идеи]

---

### Q3: Система побочных квестов

**Вопрос:** Как должны работать побочные квесты (side quests)?

**Варианты:**

**A. Статичные побочные квесты**
- Фиксированные побочные квесты от NPC
- Квесты не меняются, всегда доступны
- Простота реализации, но меньше разнообразия

**B. Динамические побочные квесты**
- Побочные квесты генерируются динамически
- Квесты меняются в зависимости от действий игрока
- Больше разнообразия, но сложнее реализация

**C. Гибридная система**
- Базовые статичные побочные квесты
- Динамические побочные квесты на основе действий игрока
- Фракционные побочные квесты от фракций
- Клановые побочные квесты от кланов

**D. Собственный вариант**
- [описание своей идеи]

---

### Q4: Ежедневные и еженедельные квесты

**Вопрос:** Как должны работать ежедневные и еженедельные квесты?

**Варианты:**

**A. Простая система повторения**
- Фиксированный набор ежедневных квестов
- Еженедельные квесты повторяются каждую неделю
- Простота реализации, но быстро надоедает

**B. Ротационная система**
- Ежедневные квесты ротируются каждый день
- Еженедельные квесты ротируются каждую неделю
- Больше разнообразия, но требует больше контента

**C. Динамическая система**
- Ежедневные квесты генерируются на основе событий мира
- Еженедельные квесты зависят от глобальных событий
- Квесты адаптируются к прогрессу игрока
- Максимальное разнообразие

**D. Гибридная система**
- Базовые ежедневные квесты (фиксированные)
- Динамические ежедневные квесты (на основе событий)
- Еженедельные квесты связаны с глобальными событиями
- Эвентовые квесты во время мировых событий

**E. Собственный вариант**
- [описание своей идеи]

---

### Q5: Награды за квесты

**Вопрос:** Какие награды должны давать квесты?

**Варианты:**

**A. Стандартные награды MMORPG**
- Опыт (XP)
- Игровая валюта
- Предметы/лут
- Репутация с фракциями

**B. Расширенная система наград**
- Опыт (XP)
- Игровая валюта
- Предметы/лут
- Репутация с фракциями
- Уникальные предметы/скины
- Доступ к контенту (зоны, способности)
- Прогресс в системах (человечность, киберпсихоз)

**C. Гибридная система**
- Стандартные награды (XP, валюта, предметы, репутация)
- Уникальные награды (предметы, скины, доступ)
- Прогресс в системах (человечность, навыки)
- Влияние на мир (изменение доступности контента)

**D. Собственный вариант**
- [описание своей идеи]

---

### Q6: Интеграция с системой выбора и последствий

**Вопрос:** Как квесты должны интегрироваться с системой выбора и последствий?

**Варианты:**

**A. Минимальная интеграция**
- Только основные квесты имеют выборы
- Побочные квесты без выборов
- Простота реализации

**B. Частичная интеграция**
- Основные квесты имеют выборы
- Некоторые побочные квесты имеют выборы
- Выборы влияют на доступный контент

**C. Полная интеграция**
- Все квесты имеют выборы (где это уместно)
- Выборы влияют на доступный контент
- Выборы влияют на репутацию с фракциями
- Выборы влияют на развитие мира
- Невозможность вернуться назад

**D. Гибридная система**
- Основные квесты всегда имеют выборы
- Побочные квесты могут иметь выборы
- Ежедневные квесты обычно без выборов (кроме эвентовых)
- Выборы влияют на доступный контент, репутацию, мир
- Связь с системой ветвления сюжета

**E. Собственный вариант**
- [описание своей идеи]

---

### Q7: Интеграция с фракциями и кланами

**Вопрос:** Как квесты должны интегрироваться с фракциями и кланами?

**Варианты:**

**A. Отдельные квесты фракций**
- Фракции предлагают свои квесты
- Квесты влияют на репутацию с фракцией
- Квесты независимы от основных квестов

**B. Интеграция в основные квесты**
- Основные квесты могут требовать выбор фракции
- Выбор фракции влияет на доступный контент
- Квесты фракций могут быть частью основного сюжета

**C. Гибридная система**
- Отдельные квесты фракций (для репутации и наград)
- Интеграция выбора фракции в основные квесты
- Клановые квесты для совместного прохождения
- Влияние на политику фракций и кланов

**D. Собственный вариант**
- [описание своей идеи]

---

### Q8: Интеграция с системой лиг

**Вопрос:** Как квесты должны интегрироваться с системой лиг (временная линия 2020-2090)?

**Варианты:**

**A. Независимые квесты**
- Квесты не привязаны к временной линии лиги
- Квесты доступны всегда независимо от периода лиги
- Простота реализации

**B. Привязанные к временной линии**
- Квесты появляются в зависимости от периода лиги
- Квесты отражают технологии и события периода
- Квесты меняются между лигами

**C. Гибридная система**
- Базовые квесты доступны всегда
- Квесты временной линии появляются в зависимости от периода
- Уникальные квесты для каждой лиги
- Связь с концепцией симуляции и раскрытием

**D. Собственный вариант**
- [описание своей идеи]

---

## Источники вдохновения

- **BALDUR GATE 3** - система ветвления сюжета, выборы влияют на сюжет
- **The Witcher 3 Wild Hunt** - глубокая система квестов, побочные квесты высокого качества
- **OBLIVION/SKYRIM** - система квестов фракций, академий
- **CYBERPUNK 2077** - квесты в киберпанк сеттинге, выборы и последствия

---

## Связанные документы

- [Ветвление сюжета](./branching-stories.md) - система ветвления сюжета и последствий
- [Диалоги](./dialogues.md) - система диалогов
- [Романтические линии](./romances.md) - романтические линии

---

## История изменений

- v1.0.0 (2025-11-03) - Создание документа с вопросами для брейншторма

