...
- Для пакетной проверки каталога используй `pwsh -NoProfile -File API-SWAGGER/scripts/validate-swagger.ps1 -ApiDirectory API-SWAGGER/api/v1/<microservice>/<domain>/`
...
| **narrative-service** | 8087 | Нарратив | `/api/v1/narrative/*` | `api/v1/narrative/dialogues/` |
| **admin-service** | 8088 | Администрирование | `/api/v1/admin/*` | `api/v1/admin/moderation/` |

> **Важно:** при заполнении `info.x-microservice` используем только перечисленные микросервисы.  
> `name` = одно из `auth-service`, `character-service`, `gameplay-service`, `social-service`, `economy-service`, `world-service`, `narrative-service`, `admin-service`; `port` — из таблицы; `domain` — корневой домен (`auth`, `characters`, `gameplay`, `social`, `economy`, `world`, `narrative`, `admin`). Поддомены (например, `mail`, `inventory`) указываем только в `base-path` (`/api/v1/social/mail`) и `directory` (`api/v1/social/mail/`).
