...
**api-readiness-notes:** Документ содержит детальные решения по сочетанию шутера/MMORPG, стилю стрельбы, TTK, системе урона, движению и стрельбе, покрытию и укрытиям (v1.1.0). TODO только для детализации отдачи и прицеливания (не блокирует API). Готов к проверке на готовность к API разработке.
