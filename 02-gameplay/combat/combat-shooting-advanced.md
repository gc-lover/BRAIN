...
### 3.1.1 Экипировка и броня

- **Весты и нагрудники:** добавляют модульные слоты (например, боекомплект, стабилизаторы для Curved Shot).
- **Перчатки:** повышают точность, уменьшают отдачу; версии с сенсорными контактами улучшают ricochet.
- **Шлемы/визоры:** Smart HUD, тепловизор, расчёт траекторий (поддержка Smart Scope).
- **Рюкзаки/подсумки:** дополнительные батареи для имплантов, материалы для крафта в поле.

Каждый элемент экипировки может содержать **модули поддержки** (например, Cooling Module для тяжёлого оружия, EMP-Shield для защиты от статус-эффектов).
